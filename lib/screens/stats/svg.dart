...
class _SvgCharacterState extends State<SvgCharacter> {
  Future<Map<String, dynamic>> getCharacterData(String character) async {
    ...
    final result = await db
        .rawQuery('select * from stroke_info where character = ?', [character]);
    if (result.isEmpty) {
      throw Exception('Character not found');
    }

    final characterData =
        Map<String, dynamic>.from(result.first); // Make a copy of the result
...
class _DisplaySvgCharacterState extends State<_DisplaySvgCharacter> {
  ...
  _StrokeInfo? _findClickedStroke(Offset position) {
    ...
    matrix.invert();
    Vector3 localPoint =
        matrix.transform3(Vector3(position.dx, position.dy, 0));
    ...
    for (int i = 0; i < painter.strokes.length; i++) {
      ...
      if (path.contains(localOffset)) {
        ...
        return _StrokeInfo(
            i, _decomposedChars[componentIndex[0]][componentIndex[1]]);
      }
    }
    return null;
  }
